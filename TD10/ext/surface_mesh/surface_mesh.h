...
public: //-------------------------------------------- constructor / destructor
    ...
    inline Surface_mesh& operator=(const Surface_mesh& rhs);

    /// assign \c rhs to \c *this. does not copy custom properties.
    inline Surface_mesh& assign(const Surface_mesh& rhs);
...
public: //--------------------------------------------------- property handling
    ...
    inline void property_stats() const;
...
public: //--------------------------------------------- higher-level operations
    ...
    inline bool is_triangle_mesh() const;

    /// returns whether the mesh a quad mesh. this function simply tests
    /// each face, and therefore is not very efficient.
    inline bool is_quad_mesh() const;
    ...
    inline bool is_collapse_ok(Halfedge h);
    ...
    /** Subdivide the edge \c e = (v0,v1) by splitting it into the two edge
     ...
     \sa insert_vertex(Edge, Vertex)
     */
    inline Halfedge insert_vertex(Halfedge h, Vertex v);
    ...
    inline Halfedge insert_edge(Halfedge h0, Halfedge h1);
    ...
    /** Check whether flipping edge \c e is topologically
     ...
     \sa flip(Edge)
     */
    inline bool is_flip_ok(Edge e) const;
    ...
    inline Halfedge find_halfedge(Vertex start, Vertex end) const;

    /// find the edge (a,b)
    inline Edge find_edge(Vertex a, Vertex b) const;

    /// deletes the vertex \c v from the mesh
    inline void delete_vertex(Vertex v);

    /// deletes the edge \c e from the mesh
    inline void delete_edge(Edge e);

    /// deletes the face \c f from the mesh
    inline void delete_face(Face f);
...
public: //------------------------------------------ geometry-related functions
    ...
    inline void update_face_normals();

    /// compute normal vector of face \c f.
    inline Normal compute_face_normal(Face f) const;

    /// compute vertex normals by calling compute_vertex_normal(Vertex) for each vertex.
    inline void update_vertex_normals();

    /// compute normal vector of vertex \c v.
    inline Normal compute_vertex_normal(Vertex v) const;
...
private: //--------------------------------------------------- helper functions
    ...
    inline void adjust_outgoing_halfedge(Vertex v);

    /// Helper for halfedge collapse
    inline void remove_edge(Halfedge h);

    /// Helper for halfedge collapse
    inline void remove_loop(Halfedge h);

    /// are there deleted vertices, edges or faces?
    inline bool garbage() const { return garbage_; }



private: //------------------------------------------------------- private data

    friend inline bool read_poly(Surface_mesh& mesh, const std::string& filename);
...
inline bool read_mesh(Surface_mesh& mesh, const std::string& filename);
inline bool read_off(Surface_mesh& mesh, const std::string& filename);
inline bool read_obj(Surface_mesh& mesh, const std::string& filename);
inline bool read_stl(Surface_mesh& mesh, const std::string& filename);

inline bool write_mesh(const Surface_mesh& mesh, const std::string& filename);
inline bool write_off(const Surface_mesh& mesh, const std::string& filename);
inline bool write_obj(const Surface_mesh& mesh, const std::string& filename);
