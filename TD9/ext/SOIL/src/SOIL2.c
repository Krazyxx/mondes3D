...
unsigned int
    SOIL_load_OGL_HDR_single_cubemap
    (
        const char *filename,
        const char face_order[6],
        unsigned int reuse_texture_ID,
    ...
    )
{
    ...
    {
        /*	try to load the image (only the HDR type) */
        img = stbi_loadf(filename, &width, &height, &channels, 0);
    }else{
        return SOIL_load_OGL_single_cubemap(filename,face_order,0,reuse_texture_ID,flags);
    }
    ...
    if( (width != 6*height) &&
        (6*width != height) )
    {
        SOIL_free_image_data_float(img );
        result_string_pointer = "Single cubemap image must have a 6:1 ratio";
        return 0;
    }
    ...
    sz = dw+dh;
    sub_img = (float *)malloc( sz*sz*channels*sizeof(float) );
    /*	do the splitting and uploading	*/
    tex_id = reuse_texture_ID;

    if(tex_id==SOIL_CREATE_NEW_ID)
    ...
    for( i = 0; i < 6; ++i )
    {
        ...
        for( y = i*dh; y < i*dh+sz; ++y )
        {
            ...
            {
                sub_img[idx++] = img[y*width*channels+x];
            }
        }
...
//                SOIL_MAX_CUBE_MAP_TEXTURE_SIZE );
        ...
        glTexImage2D(cubemap_target, 0, GL_RGB32F_ARB, sz, sz, 0, GL_RGB, GL_FLOAT, sub_img);
        check_for_GL_errors( "glTexImage2D" );

    }
    /*	and nuke the image and sub-image data	*/
    SOIL_free_image_data_float( sub_img );
    /*	and return the handle, such as it is	*/
    return tex_id;
}
